"""
Miscellaneous utilities for pretraining and downstream tasks.
"""
...
def adjust_learning_rate(optimizer, i_iter, max_iter, lr, power=0.9):
    """Decay the learning rate based on schedule"""
    cur_lr = lr * ((1 - float(i_iter) / max_iter) ** power)
...
def get_next_run_number(base_path):
    """Get the next run number for experiments."""
    if not os.path.exists(base_path):
        return 0
    
    existing_runs = [d for d in os.listdir(base_path) if d.startswith('run_') and d[4:].isdigit()]
